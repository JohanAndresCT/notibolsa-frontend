...
export default function App() {
 ...
 const handleAnalyze = async () => {
  ...
  try {
    ...
    console.log('Enviando consulta al backend:', `${API_URL}/aggregate?${params}`);
    const response = await fetch(`${API_URL}/aggregate?${params}`);
    console.log('Respuesta HTTP:', response.status, response.statusText);

    if (!response.ok) {
      console.error('Error HTTP al obtener los datos:', response.status, response.statusText);
    ...
    if (Array.isArray(result.combined)) {
      ...
      newsCount = typeof result.commoncrawl?.count === 'number'
        ? result.commoncrawl.count
        : combined.reduce((acc, item) => acc + (item.news || 0), 0);
      linesCount = typeof result.commoncrawl?.lines_count === 'number'
      ...
      if (result.commoncrawl) {
        if (result.commoncrawl.matching_urls) {
          urls = Array.isArray(result.commoncrawl.matching_urls) ? result.commoncrawl.matching_urls : [];
          newsCount = typeof result.commoncrawl.count === 'number' ? result.commoncrawl.count : urls.length;
          linesCount = typeof result.commoncrawl.lines_count === 'number' ? result.commoncrawl.lines_count : newsCount;
        } else {
          newsCount = typeof result.commoncrawl.news_count === 'number' ? result.commoncrawl.news_count : 0;
          linesCount = typeof result.commoncrawl.lines_count === 'number' ? result.commoncrawl.lines_count : newsCount;
        }
      } else {
        if (result.matching_urls) {
          urls = Array.isArray(result.matching_urls) ? result.matching_urls : [];
          newsCount = typeof result.count === 'number' ? result.count : urls.length;
      ...
      const colcapArr: { date: string; colcap: number }[] = Array.isArray(result.colcap)
        ? result.colcap.map(
        ...
        : [];
      const noticiasMap = new Map<string, number>(noticiasArr.map(item => [item.date, item.news]));
      const colcapMap = new Map<string, number>(colcapArr.map(item => [item.date, item.colcap]));
      ...
      combined = allDates.map((date: string) => ({
        ...
        colcap: typeof colcapMap.get(date) === 'number' ? colcapMap.get(date)! : 0
      }));
    }
          ...
          <>
            ...
            <div className="mt-12 space-y-8">
              ...
              <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                ...
                <ResultsTable data={data} newsCount={newsCount} linesCount={linesCount} urls={urls} />
              </div>
            </div>
          </>
        )}
